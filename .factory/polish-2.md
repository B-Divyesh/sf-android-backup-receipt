...
Repair commits: `680a15d4b957692c4fc737e8a97e88e28e00b964` and final source audit `97c8fe9b7ef5c9903409695203d94878af2fd8d7`

Production: <https://android-backup-receipt.sociobot.in>

Final static deployment: `556fb5c4-c0f8-4410-acee-fba50faf2e26`

Cold production report: `.factory/qa-evidence/polish-2/live/report.json`

Primary screenshots: `.factory/qa-evidence/polish-2/live/final-home-mobile.png`, `.factory/qa-evidence/polish-2/live/final-demo-mobile.png`, `.factory/qa-evidence/polish-2/live/final-home-after-back.png`, and `.factory/qa-evidence/polish-2/live/demo-offline.png`.
...
| --- | --- | --- |
| F-1-1 | Kept the demo-first receipt: 50%, 2 matched, 1 missing, 1 changed, issue rows, and downloads all fit in the initial phone viewport. | Test `@claim:demo-sample-receipt`; screenshot `.factory/qa-evidence/polish-2/live/final-demo-mobile.png`; live `/demo`, report `demo.receipt` y=178–699.7 and exports ending at y=634.7. |
| F-1-2 | Kept the exact first-screen outcome “showing which selected files match”; no restoreability claim remains. | Test `@claim:comparison-manifest`; screenshot `.factory/qa-evidence/polish-2/live/final-home-mobile.png`; cold live `/` copy check. |
...
| F-1-11 | Replaced pushState-plus-reload with ordinary `/demo` navigation and strengthened Back assertions for title, h1, demo state, hidden UI, announcement, and focus. | Same route test; screenshot `.factory/qa-evidence/polish-2/live/final-home-after-back.png`; live report `back` proves the complete restored home state. |
| F-1-12 | Kept the README opening split into two short sentences. | Copy audit `.factory/copy-audit.md`; screenshot `.factory/qa-evidence/polish-2/live/final-home-mobile.png`; cold live `/` and repository README check. |
...
| F-1-15 | Kept outcomes before JSON, CSV, SHA-256, APK, AAB, and other technical terms. | Copy audit plus `@claim:responsive-keyboard`; screenshot `.factory/qa-evidence/polish-2/live/final-home-mobile.png`; cold live `/` read-through. |
| F-1-16 | Kept one vocabulary: phone folder, backup folder, matched, saved folder record, receipt, and Migration Kit. | Copy-audit terminology table plus `@claim:multi-folder-receipt`; screenshot `.factory/qa-evidence/polish-2/live/final-demo-mobile.png`; cold live `/demo` read-through. |
...
| F-1-18 | Kept the self-contained “Missing or changed files” heading. | Test `@claim:demo-sample-receipt`; screenshot `.factory/qa-evidence/polish-2/live/final-demo-mobile.png`; cold live `/demo`. |
...
| F-1-21 | Kept the concrete footer line “Compare selected phone and backup folders on this device.” | Route-shell test; screenshot `.factory/qa-evidence/polish-2/live/final-home-mobile.png`; cold live crawl confirms it on every route. |
| F-1-22 | Kept artwork provenance only in `.factory/design.md`, not landing copy. | Copy audit; screenshot `.factory/qa-evidence/polish-2/live/final-home-mobile.png`; cold live `/` footer check. |
...
| F-2-1 | Ordinary document navigation now gives `/` and `/demo` distinct documents; Back restores the complete home state instead of stale demo DOM. | Strengthened route test; screenshot `.factory/qa-evidence/polish-2/live/final-home-after-back.png`; live report `back` records home title/h1, `demo=false`, hidden banner/receipt, correct announcement, and focused h1. |
| F-2-2 | Reset and exit delete the entire demo IndexedDB database and every demo-prefixed local key. Reset then reseeds only the four-file sample; real data is untouched. | Test `@claim:demo-reset-isolation`; screenshot `.factory/qa-evidence/polish-2/live/final-demo-mobile.png`; live report `reset` records 2 active/0 history after reset and no demo database after exit, with the real license preserved. |
...
| F-2-4 | Added an all-public-route claim for no analytics, ads, remote fonts, or third-party runtime scripts. It inspects requests and loaded HTML/JS/CSS. | Test `@claim:no-tracking-runtime`; screenshot `.factory/qa-evidence/polish-2/live/final-home-mobile.png`; live report has `requests: []` across all public routes. |
...
| F-2-8 | Receipt warnings now generate grammatical singular/plural parts. The sample says “1 file is missing and 1 has changed.” | Tests `@claim:demo-sample-receipt` and `@claim:resume-reset`; screenshot `.factory/qa-evidence/polish-2/live/final-demo-mobile.png`; live report `demo.warning` records the exact sentence. |
...
| F-2-10 | Replaced promotional “honest limits” with “Read what this does not check”. | Copy audit plus `@claim:responsive-keyboard`; screenshot `.factory/qa-evidence/polish-2/live/final-home-mobile.png`; cold live first-screen exact-copy check. |
| F-2-11 | Removed the 390 px/keyboard QA instruction from the user guide and moved test coverage to the developer section. | Test `@claim:responsive-keyboard`; screenshot `.factory/qa-evidence/polish-2/live/final-home-mobile.png`; repository README check and cold live mobile probe. |
...
- Final fresh clone `/tmp/abr-polish2-final.TzhLtj`: all 20 exact commands from `.factory/claims.json` passed separately; each selected one tagged test.
...
- `dist/`: 33,491-byte JS (11.90 kB gzip) and 18,503-byte CSS (4.60 kB gzip).
...
- Deployed and local `dist/index.html` are byte-identical: SHA-256 `8f247628adda217645c07d5a13bbe4341283dfc2982f52f34e5168243dd02cbc`.
- Android Actions run `33271561492` succeeded for the final repair commit. Release `android-v1.0.3-build-11` contains the APK and AAB; both match `SHA256SUMS` and pass ZIP integrity.
